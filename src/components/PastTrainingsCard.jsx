...
const PastTraininigsCard = () => {
  ...
  const handleSave = (id) => {
    setTrainings(prev =>
      prev.map(training =>
        training.id === id ? { ...training, description: tempDescription } : training
      )
    );
    setEditingId(null);
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-[25px] font-semibold text-slate-800">Předešlé tréninky</h2>
      </div>
      <div className={`grid gap-4 ${trainings.length === 1 ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2"}`}>
          ...
          >
            <div>
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center justify-center">
                  ...
                  <h3 className="font-medium text-[19px] text-gray-600">{training.location}</h3>
                </div>
              </div>
              ...
              <div className="text-slate-600 space-y-2 text-sm">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-[14px]">
                    {training.dayOfTheWeek} {new Date(training.date).toLocaleDateString()} • {training.time}
                  </span>
                </div>
                ...
                <div className="flex flex-col">
                  <div className="flex items-start mt-2">
                    ...
                    {editingId === training.id ? (
                      <textarea
                        className="border border-gray-300 rounded p-2 text-[14px] w-full"
            ...
            <div className="absolute bottom-1 right-1 flex gap-2">
              {editingId === training.id ? (
                ...
                <button
                  className="text-sm px-3 py-1 text-gray-500 border border-gray-300 rounded"
