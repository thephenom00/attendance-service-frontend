...
const AttendanceShoolCard = ({ training }) => {
  ...
  return (
    <div className="w-full max-w-xl bg-white rounded-xl shadow-md p-6 border border-slate-200 hover:border-dojo-blue/30 transition">
      {/* Top Section */}
      ...
      <div className="flex items-start text-slate-600">
        ...
        <p className="text-[15px] whitespace-pre-wrap">
          {training.description
            ? training.description
            : "Popis tréninku zatím není k dispozici dawdwwdwdadawawddawadwadwawdadwadw. dwaw dadw a wdaw da wdawd. dw."}
        </p>
      </div>
      ...
      <div className="flex justify-end mt-4">
        {editingId === training.id ? (
          <>
            <button
              className="text-sm px-2 py-1 mt-4 text-white bg-emerald-500 rounded hover:bg-emerald-600"
              onClick={() => handleSave(training.id)}
            >
              Save
            </button>
            <button
              className="text-sm px-2 py-1 ml-1 mt-4 text-white bg-rose-500 rounded hover:bg-rose-600"
          ...
          <button
            className="text-sm px-2 py-1 mt-4 text-gray-500 hover:cursor-pointer"
